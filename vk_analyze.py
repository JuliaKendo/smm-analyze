...
from smm_lib import get_api_parameters, get_restriction_date
...
def get_post_commentators(params, restriction_date):
    ...
    for offset in itertools.count(0, step):
        ...
        yield from [comment['from_id'] for comment in post_comments['response']['items'] if comment['date'] > restriction_date and 'from_id' in comment.keys()]
...
def fetch_vk_commentators_rating(vk_token, vk_account, number_posts):
    ...
    params = get_api_parameters(vk_token, domain=vk_account, v='5.120')
    vk_posts = fetch_vk_posts(params, number_posts)
    for post_id, owner_id in tqdm(vk_posts, desc="Обработанно", unit=" постов VK"):
